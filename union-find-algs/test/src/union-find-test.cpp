...
TEST(UnionFind, Union_AreConnected) {
    ...
    for (auto i : v)
        for (auto j : v)
            if (i == j) ASSERT_TRUE(uf_tester.AreConnected(i, j));
            else ASSERT_FALSE(uf_tester.AreConnected(i, j));

    uf_tester.Union(0, 1);
    ASSERT_TRUE(uf_tester.AreConnected(0, 1));
    ASSERT_FALSE(uf_tester.AreConnected(0, 2));
    uf_tester.Union(0, 2);
    ASSERT_TRUE(uf_tester.AreConnected(0, 2));
}
